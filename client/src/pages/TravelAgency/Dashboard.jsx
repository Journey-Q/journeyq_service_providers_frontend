...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      processDashboardData(bookingsData, vehiclesData, driversData, reviewsData, promotionsData);
    } catch (err) {
  ...
  const processDashboardData = (bookings, vehicles, drivers, reviews, promotions) => {
    // Process bookings
    ...
    const recentRentalsList = validBookings
      ...
      .map(booking => ({
        ...
        amount: formatAmount(booking.estimatedTotalAmount || booking.totalAmount),
    ...
    setPromotions(validPromotions.filter(p => 
      p.status?.toLowerCase() === 'advertised' || p.status?.toLowerCase() === 'approved'
    ).slice(0, 2));
  ...
  const calculateStats = (bookings, vehicles, reviews) => {
    ...
    const periodRevenue = filterByPeriod(completedBookings, selectedPeriod)
      .reduce((sum, booking) => sum + (booking.estimatedTotalAmount || booking.totalAmount || 0), 0);
    ...
    const usageRate = totalVehicles > 0 ? Math.round(((totalVehicles - availableVehicles) / totalVehicles) * 100) : 0;
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-3xl font-bold text-gray-800">{stats.rentals}</p>
                  <p className="text-xs text-gray-500 capitalize">{selectedPeriod}</p>
                </div>
            ...
            <section className="bg-white rounded-xl shadow-md border border-gray-100 lg:col-span-2">
              ...
              <div className="p-6">
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-2">
                          <h3 className="font-semibold text-gray-800">{rental.customer}</h3>
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(rental.status)}`}
                          >
                            {rental.status}
                          </span>
                        </div>
                ...
                <div className="mt-6 pt-4 border-t border-gray-100">
                  <button className="w-full px-4 py-3 bg-[#1F74BF] text-white rounded-lg hover:bg-[#2953A6] transition-colors font-medium">
                    View All Rentals
                  </button>
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    {vehicles.length > 0 ? vehicles.map(vehicle => (
                      <div key={vehicle.id} className="flex items-center justify-between">
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-medium ${getVehicleStatusColor(vehicle.status)}`}
                        >
                      ...
                      <div key={driver.id} className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{driver.experience} years exp</p>
                          </div>
                        </div>
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-medium ${getDriverStatusColor(driver.status)}`}
                        >
                          {driver.status}
                        </span>
                      </div>
                    ...
                    {reviews.length > 0 ? reviews.map(review => (
                      <div key={review.id} className="bg-gray-50 rounded-lg p-4">
                        ...
                        <p className="text-sm text-gray-600 line-clamp-2">
                          {review.reviewText || review.review || 'No review content'}
                        </p>
                      </div>
                    )) : (
                      <div className="text-center py-4 text-gray-500">
                ...
                <div className="p-6">
                  <div className="space-y-4">
                    {promotions.length > 0 ? promotions.map(promotion => (
                      <div key={promotion.id} className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-100">
                        <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <button className="w-full px-4 py-2 bg-[#0B9ED9] text-white rounded-lg hover:bg-[#0891C7] transition-colors font-medium">
                      Create Promotion
                    </button>
                  </div>
                </div>
