...
const InsertVehicle = ({ onClose, onVehicleAdded }) => {
  ...
  const prepareVehicleDataForAPI = async (serviceProviderId) => {
    ...
    if (formData.imageFile) {
      try {
        ...
        vehicleImageUrl = await CloudinaryStorageService.uploadImage(formData.imageFile, fileName);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.imageFile) {
      setError('Please upload an image for the vehicle.');
      return;
    }
    ...
    try {
      ...
      const response = await TravelVehicleService.createVehicle(apiData);
      
      console.log('Vehicle created successfully:', response);
      setUploadProgress('Vehicle created successfully! 🎉');
      ...
      console.error('Error creating vehicle:', apiError);
      setError(apiError.message || 'Failed to add vehicle. Please try again.');
      setUploadProgress('');
    } finally {
