...
import { FiDollarSign, FiCalendar, FiUser, FiInfo, FiFilter, FiCreditCard, FiMail, FiPhone, FiTruck } from 'react-icons/fi';

const PaymentHistory = () => {
  // Sample payment data - only completed transactions with service IDs
  const allPayments = [
    {
      id: 1,
      serviceId: 'TRVL-2025-0615-001',
      client: { 
        name: 'John D. Silva', 
        service: 'Colombo to Kandy Van Transfer',
    ...
    {
      id: 2,
      serviceId: 'TRVL-2025-0528-002',
      client: { 
        name: 'Sarah Johnson', 
        service: 'Airport to City Car Transfer',
    ...
    {
      id: 3,
      serviceId: 'TRVL-2025-0405-003',
      client: { 
        name: 'Michael Brown', 
        service: 'Galle to Yala Bus Transfer',
    ...
    {
      id: 4,
      serviceId: 'TRVL-2025-0320-004',
      client: { 
        name: 'Alice M. Smith', 
        service: 'Kandy to Nuwara Eliya Van Transfer',
    ...
    {
      id: 5,
      serviceId: 'TRVL-2025-0215-005',
      client: { 
        name: 'Robert K. Lee', 
        service: 'Colombo City Tour',
      ...
      <main className="flex-1 p-6 lg:p-8">
        <header className="mb-8">
          ...
          <p className="text-slate-600 mt-2">
            View all completed payments for transportation services
          </p>
        </header>
                  ...
                  filteredPayments.map((payment) => (
                    <tr key={payment.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-sm text-slate-500">{payment.client.service}</div>
                      </td>
                ...
                return payment ? (
                  <div className="p-6">
                    ...
                    <div className="mb-4 p-3 bg-blue-100 rounded-lg">
                      <div className="font-semibold text-blue-600">Payment #: {payment.id}</div>
                    </div>

                    {/* Related Service */}
                    <div className="mb-4 p-3 bg-green-50 rounded-lg border border-green-200">
                      <div className="text-sm text-green-700">Related Service</div>
                      <div className="font-mono text-green-600 font-semibold">{payment.serviceId}</div>
                    </div>

                    {/* Client Details */}
                    <div className="space-y-4 mb-6">
                      <h4 className="font-semibold text-slate-800 text-lg border-b border-gray-100 pb-2">Client Information</h4>
                      
                      <div className="space-y-3">
                        <div className="flex items-center gap-3">
                          <FiUser className="text-blue-600 w-5 h-5 flex-shrink-0" />
                          <div>
                            <div className="font-semibold text-slate-800">{payment.client.name}</div>
                            <div className="text-sm text-slate-600">{payment.client.nationality}</div>
                          </div>
                        </div>
                        
                        <div className="flex items-center gap-3">
                          <FiMail className="text-blue-600 w-5 h-5 flex-shrink-0" />
                          <div className="text-slate-700">{payment.client.email}</div>
                        </div>
                        
                        <div className="flex items-center gap-3">
                          <FiPhone className="text-blue-600 w-5 h-5 flex-shrink-0" />
                          <div className="text-slate-700">{payment.client.phone}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Service Details */}
                    <div className="space-y-4 mb-6">
                      <h4 className="font-semibold text-slate-800 text-lg border-b border-gray-100 pb-2">Service Information</h4>
                      
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <span className="text-sm text-slate-600">Service Type:</span>
                          <div className="font-semibold text-slate-800">{payment.client.service}</div>
                        </div>
                        
                        <div>
                          <span className="text-sm text-slate-600">Service Date:</span>
                          <div className="font-semibold text-slate-800">{payment.serviceDate}</div>
                        </div>

                        <div>
                          <span className="text-sm text-slate-600">Passengers:</span>
                          <div className="font-semibold text-slate-800">{payment.passengers}</div>
                        </div>
